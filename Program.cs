    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine("Resulting polynomial will be of the order - " + Polynomial_class.CalculateOrder(matrix) + "\nCalculated polynomial:");
                double[] coefficients = matrix.GetResults();
                Console.WriteLine(Polynomial_class.Format(coefficients));
                Dictionary<int, double> calculatedPolynomial = Polynomial_class.Calculate(coefficients);
                foreach (KeyValuePair<int, double> value in calculatedPolynomial)
                    Console.WriteLine($"f({value.Key}) = {value.Value:0.000}");
                Console.WriteLine("Derivative:\n" + Polynomial_class.CalculateDerivative(coefficients));
                double inititalGuess = 2;
                Console.WriteLine("Looking for a root with initial guess 2");
                Console.WriteLine("Root found for x = " + Polynomial_class.CalculateRoot(coefficients, inititalGuess).ToString("0.00000"));
            }
            catch (Exception e)
            {
